              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-10 items-start md:items-center">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <span className="text-emerald-400 font-mono text-xs tracking-widest font-bold">
                      DIR-01
                    </span>
                  ...
                  <p className="text-slate-400 text-lg leading-relaxed max-w-xl">
                    Accelerating proposal lifecycles by neutralizing physical
                    logistics and implementing an
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="flex items-end justify-between mt-12">
              <div className="flex -space-x-3">
                {["I", "R", "A"].map((l, i) => (
                  <div
                    key={i}
                    className="w-12 h-12 rounded-full border-4 border-white bg-slate-900 flex items-center justify-center text-[10px] font-black text-white group-hover:bg-emerald-500 transition-colors"
                  >
                    {l}
                  </div>
                ))}
          ...
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {secondaryGoals.map((goal, idx) => (
              <div
                key={idx}
                className="bg-white border border-slate-100 rounded-[3rem] p-10 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 group"
              >
                <div
                  className={`w-14 h-14 rounded-2xl ${goal.bg} ${goal.color} flex items-center justify-center mb-8 group-hover:scale-110 transition-transform`}
                >
                  {goal.icon}
                </div>
                <div className="space-y-4">
