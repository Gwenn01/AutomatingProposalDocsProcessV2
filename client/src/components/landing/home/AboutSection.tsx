...
const AboutSection: React.FC = () => {
  const workflowStatuses = [
    ...
    },
    {
      ...
      sub: "In Process",
      color: "#f59e0b",
      icon: <RotateCcw size={14} className="animate-spin-slow" />,
      detail: "Active",
    },
    {
      status: "For Revisions",
      sub: "Action Required",
      color: "#ea580c",
  ...
  return (
    <section
      id="about"
      className="relative py-12 lg:py-20 bg-[#fafafa] overflow-hidden"
    >
      {/* Background Ambience */}
            ...
            <div className="relative transform-gpu transition-all duration-700 hover:rotate-y-[-5deg]">
              ...
              <div className="bg-white rounded-[3rem] p-2 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.12)] border border-white relative z-10 overflow-hidden">
                <div className="bg-slate-50/50 rounded-[2.5rem] border border-slate-100 overflow-hidden">
                  ...
                  <div className="px-8 py-6 bg-white border-b border-slate-100 flex justify-between items-center">
                    ...
                    <div className="px-3 py-1 bg-emerald-50 rounded-full text-emerald-600 text-[10px] font-bold tracking-tighter">
                      Live v2.4
                    </div>
                  </div>
                  ...
                  <div className="p-8 space-y-4">
                    {workflowStatuses.map((item, i) => (
                      <div
                        key={i}
                        className="group/step relative flex items-center gap-6"
                      >
                        {/* Connecting Line */}
                        ...
                        {/* Icon Node */}
                        <div
                          className="relative flex-shrink-0 w-10 h-10 rounded-2xl bg-white shadow-sm border border-slate-100 flex items-center justify-center transition-all duration-500 group-hover/step:scale-110 group-hover/step:border-emerald-200"
                        ...
                        <div className="flex-grow bg-white/60 p-4 rounded-2xl border border-white group-hover/step:border-emerald-100 group-hover/step:bg-white transition-all duration-300 flex justify-between items-center shadow-sm hover:shadow-md">
                          ...
                          <div className="text-right">
                            <span className="text-[9px] font-black uppercase text-slate-300 group-hover/step:text-emerald-500 transition-colors tracking-widest">
                              {item.detail}
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100">
                      <div className="flex items-center justify-between px-4 py-3 bg-red-50/50 rounded-2xl border border-red-100/50 group hover:bg-red-50 transition-colors">
                        ...
                        <span className="text-[10px] text-red-400 font-medium italic underline underline-offset-2">
                          View Reason
                        </span>
                      </div>
                    </div>
              ...
              <div className="absolute -bottom-10 -right-10 bg-white/80 backdrop-blur-xl border border-white p-6 rounded-[2rem] shadow-2xl z-20 hidden lg:block animate-bounce-slow">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs font-black text-slate-900 tracking-tighter mt-2">
                    Verified Process
                  </p>
                </div>
              </div>
              ...
              {[
                {
                  icon: <Leaf />,
                  title: "Eco-Archive",
                  desc: "No more printing. Fully digital proposal vault.",
                },
                {
                  icon: <Clock />,
                  title: "Live Sync",
                  desc: "Real-time updates as reviewers sign off.",
                },
              ].map((feature, i) => (
