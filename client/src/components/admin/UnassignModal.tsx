...
import { getAssignedReviewers, unassignReviewer, getAllReviewers, type ReviewerAssignment, type ApiUser } from "@/utils/admin-api";
...
const UnassignModal: React.FC<UnassignModalPropos> = ({ isOpen, onClose, data, onUpdate }) => {
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-[480px] rounded-[32px] shadow-2xl p-8">
                ...
                {isLoading ? (
                    ...
                    const reviewer = reviewers.find(r => r.id === assignment.reviewer);
                    ...
                    return (
                        ...
                        <div className="text-left">
                            ...
                            {reviewer?.profile.name || `Reviewer ID: ${assignment.reviewer}`}
                            </p>

                            <p className="text-xs text-slate-400">
                            {reviewer?.profile.department || "Department not available"}
                            </p>
                        ...
                        >
                            <Trash2 size={16} />
                            {isSubmittingId === assignment.id ? "Removing..." : "Unassign"}
                        </button>
                        </div>
                    );
                    })
                ) : (
                    <p className="text-center text-slate-400 py-6">No reviewers assigned.</p>
                )}
                </div>
            </div>
            </div>,
