...
export const ProgramProposalForm = ({ data, onChange, onNext, isSubmitting }: ProgramFormProps) => {
  ...
  const budgetTotal = parseFloat((data as any).program_budget_total) || 0;
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="py-5">
            ...
            <button
              onClick={() => upd('projects', [...data.projects, defaultProjectItem(data.projects.length)])}
      ...
      {/* ── X. Budgetary Requirement — single total amount only ── */}
      <Card>
        <SectionHeader
          title="X. Budgetary Requirement"
        ...
        <div className="mt-4 max-w-lg">
          ...
          {budgetTotal > 0 && (
            <p className="mt-2 text-xs text-emerald-600 font-semibold">
              ✓ {budgetTotal.toLocaleString('en-PH', { style: 'currency', currency: 'PHP' })} allocated to this program.
              Projects and activities must stay within this total.
            </p>
          )}
        </div>
      </Card>
