...
import { ChevronRight, X, FileText, FolderOpen, Activity, Download, Loader2 } from "lucide-react";
...
const ExportPdfButton: React.FC<ExportPdfButtonProps> = ({ contentRef, filename, disabled }) => {
...
const DocumentViewerModal: React.FC<DocumentViewerModalProps> = ({
  ...
  const isExportDisabled = (): boolean => {
    ...
    if (activeTab === "project")  return !selectedProject  || projectDetailLoading;
    if (activeTab === "activity") return !selectedActivity || activityDetailLoading;
    return true;
  };
          ...
          <div className="flex items-center gap-2 mr-6 bg-white/10 backdrop-blur-md p-1.5 rounded-xl border border-white/15 shadow-inner">
            ...
            >
              ...
              {projectList.length > 0 && (
                <span className={`ml-1 text-[10px] px-2 py-[2px] rounded-full font-bold transition-all
