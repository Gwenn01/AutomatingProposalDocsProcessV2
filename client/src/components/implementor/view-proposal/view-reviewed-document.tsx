...
=======
    ...
    workplan: data.work_plan?.content ?? [],
    budget_requirements: data.budget_requirements?.content ?? [],
>>>>>>> 2bd426bfb92fb72d4bb49a23bdb61d44fa9c093e
  };
}
...
const ViewReviewedDocuments: React.FC<ViewReviewedDocumentsProps> = ({
  ...
  // ── Reviewed API data ─────────────────────────────────────────────────────
  const [programReviewedData, setProgramReviewedData] = useState<any | null>(
    null,
  );
  const [projectReviewedData, setProjectReviewedData] = useState<any | null>(
    null,
  );
  const [activityReviewedData, setActivityReviewedData] = useState<any | null>(
    null,
  );
  ...
  const [activityAllReviewed, setActivityAllReviewed] =
  ...
  const showCommentInputs =
    selectedHistoryVersion?.status === "current" || !selectedHistoryVersion;
  ...
  const mappedProgram = React.useMemo(
    () => mapReviewedToProgram(programReviewedData),
    [programReviewedData],
  );
  const mappedProject = React.useMemo(
    () => mapReviewedToProject(projectReviewedData),
    [projectReviewedData],
  );
  const mappedActivity = React.useMemo(
    () => mapReviewedToActivity(activityReviewedData),
    [activityReviewedData],
  );
  ...
  const projectChildId = selectedProject
    ? Number(selectedProject.child_id)
    : null;
  const projectProposalId = selectedProject
    ? Number(selectedProject.proposal_id)
    : null;
  const activityChildId = selectedActivity
    ? Number(selectedActivity.child_id)
    : null;
  const activityProposalId = selectedActivity
    ? Number(selectedActivity.proposal_id)
    : null;
  ...
  const programTitle =
    mappedProgram?.program_title ?? proposalData?.title ?? "";

  // ── Notify parent of initial load state ──────────────────────────────────
  useEffect(() => {
    onLoadingChange?.(programLoading);
  }, [programLoading, onLoadingChange]);
  ...
  useEffect(() => {
    ...
    fetchProjectList(childId)
      .then((data: ApiProjectListResponse) =>
  ...
  useEffect(() => {
    ...
    const resolvedProposalId =
      activeTab === "project"
        ? (selectedProject?.proposal_id ?? proposal_id)
        : activeTab === "activity"
          ? (selectedActivity?.proposal_id ?? proposal_id)
          : (nodeId ?? proposal_id);
  ...
  const handleSelectProject = useCallback(async (project: ProjectItem) => {
    ...
    try {
      const detail = await fetchReviewedProposal(
        project.proposal_id,
        "project",
      );
  ...
  const activeActivityReviewedData =
    historySnapshotData ?? activityReviewedData;

  // ── Resolved mapped data when a history snapshot is active ────────────────
  const activeMappedProgram = historySnapshotData
    ? mapReviewedToProgram(historySnapshotData)
    : mappedProgram;
  const activeMappedProject = historySnapshotData
    ? mapReviewedToProject(historySnapshotData)
    : mappedProject;
  const activeMappedActivity = historySnapshotData
    ? mapReviewedToActivity(historySnapshotData)
    : mappedActivity;
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-10 py-6 bg-primaryGreen border-b border-white/10">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span
                className={`inline-flex items-center px-4 py-[4px] rounded-full text-[12px] uppercase font-semibold border border-white/20 shadow-sm ${statusStyle.className}`}
              >
                {statusStyle.label}
              </span>
            </div>
              ...
              {activeTab === "project" &&
                (!selectedProject ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-gray-400 gap-3">
                    <FolderOpen size={40} className="text-gray-300" />
                    <p className="font-medium">
                      No project review data available
                    </p>
                  </div>
                ))}
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1">
              {historyLoading ? (
                ...
                activeHistory.map((item, index) => {
                  const isSelected =
                    selectedHistoryVersion?.history_id === item.history_id;
                    ...
                    <div
                      ...
                      onClick={() => {
                        if (isSelected) {
                          ...
                          const current =
                            activeHistory.find((h) => h.status === "current") ??
                            null;
                          setSelectedHistoryVersion(current);
                        } else {
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 pb-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span
                            className={`text-sm font-medium leading-snug truncate ${isSelected ? "text-emerald-800" : "text-gray-800"}`}
                          >
                            {isCurrent
                              ? "Current Version"
                        ...
                        <p
                          className={`text-[10px] mt-0.5 ${isSelected ? "text-emerald-600/70" : "text-gray-400"}`}
                        >
                          Title:{" "}
                          <span>
                            {item.program_title ||
